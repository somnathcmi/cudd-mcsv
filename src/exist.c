...
DdNode *ExistAnd(DdManager *manager,DdNode *f,DdNode *g,DdNode *cube)
{
	...
	t = ExistAnd(manager, ft, gt, cube);
	if(t == NULL) return NULL;
	Cudd_Ref(t);

	e = ExistAnd(manager, fe, ge, cube);	
	if(e == NULL)
	{ 
	...
	else
	{
		...
		{
			r = cuddUniqueInter(manager,(int)topvar,Cudd_Not(t),Cudd_Not(e));
			if(r == NULL)
			{
				Cudd_IterDerefBdd(manager,t);
			...
			r = cuddUniqueInter(manager,(int)topvar,t,e);
	}
